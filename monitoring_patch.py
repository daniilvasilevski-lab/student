#!/usr/bin/env python3
"""
Патч для обновления системы мониторинга в unified_launcher.py
"""
...
def apply_monitoring_patch():
    ...
    # 1. Заменяем метод _run_monitoring
    old_monitoring = re.compile(
        r'    def _run_monitoring\(self\):.*?(?=    def _display_status\(self, status_info\):)',
        re.DOTALL
    )
    
    new_monitoring = '''    def _run_monitoring(self):
    ...
    content = old_monitoring.sub(new_monitoring, content)
    
    # 2. Добавляем новые методы перед методом stop
    stop_method_pos = content.find('    def stop(self):')
    if stop_method_pos != -1:
        new_methods = '''    def _display_system_status(self):
    ...
    '''
        ...
        content = content[:stop_method_pos] + new_methods + content[stop_method_pos:]
